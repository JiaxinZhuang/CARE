...
class DatasetFolder(data.Dataset):
    """DatasetFolder"""
    def __init__(self, train=True, transform=None, iter_no=1, logger=None):
        self.logger = logger
        self.train = train
    ...
    def make_dataset(self, iter_no, data_dir):
        ...
        self.logger.info('Loading train from {}'.format(train_csv))
    ...
    def pil_loader(self, path):
        ...
        with open(path, 'rb') as f:
            #self.logger.notest('load image {} using pil_loader'.format(path))
            img = Image.open(f)
